...
function PhaseBar({ currentPhase }) {
  const { jumpToPhase } = useSessionStore()
          ...
          return (
            <React.Fragment key={phase}>
              ...
              >
                ...
                <span className="text-xs hidden md:block group-hover:text-white transition-colors" style={{ color: status === 'upcoming' ? '#4A5A7C' : color, fontSize: '0.6rem', whiteSpace: 'nowrap' }}>
                  {label}
                </span>
              </motion.div>
            </React.Fragment>
